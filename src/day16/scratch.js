...
const readData = () => {
    const data = fs
        ...
        .map((line) => {
            return /Valve (\w+) has flow rate=(\d+); tunnels? leads? to valves? (.+)/g.exec(line);
        })
        ...
        .reduce(
            (acc, { valve, rate, to }) => Object.assign(acc, { [valve]: { rate, to } }),
...
const main = () => {
    ...
    const valvesHavingPositiveRate = valves.filter((valve) => Boolean(data[valve].rate));
    ...
    const minTime = valves.reduce((acc, valveFrom) => {
        acc[valveFrom] = valves.reduce((acc, valveTo) => {
            acc[valveTo] = data[valveFrom].to.includes(valveTo) ? 1 : Number.POSITIVE_INFINITY;
    ...
    const recursion = ({
        ...
        for (const valve of valvesHavingPositiveRate) {
            ...
            if (newMinutesLeft <= 0 || isVisited(valveBitRepr[valve], openedBitRepr)) continue;
